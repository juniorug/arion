...
approveForMyOrg1() {
   ...
   setEnvVarsForPeer0Org1
   peer lifecycle chaincode approveformyorg -o $ORDERER_ADDRESS --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA --channelID ${CHANNEL_NAME} --name ${CHAINCODE_NAME} --version ${CHAINCODE_VERSION} \
...
commitChaincodeDefinition() {
   ...
   setEnvVarsForPeer0Org1
   peer lifecycle chaincode commit -o $ORDERER_ADDRESS --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA --channelID ${CHANNEL_NAME} --name ${CHAINCODE_NAME} \
      --peerAddresses localhost:7051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG1 --peerAddresses localhost:9051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG2 --version ${CHAINCODE_VERSION} --sequence 1 --init-required
...
chaincodeInvokeInit() {
   ...
   peer chaincode invoke -o $ORDERER_ADDRESS --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C ${CHANNEL_NAME} --name ${CHAINCODE_NAME} \
      --peerAddresses localhost:7051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG1 --peerAddresses localhost:9051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG2 --isInit -c '{"Args":[]}'
   echo "===================== Succesfully Initilized the chaincode===================== "
}

chaincodeAddActor() {
   echo "===================== Started Add Actor Chanicode Function===================== "
   setEnvVarsForPeer0Org1
   peer chaincode invoke -o $ORDERER_ADDRESS --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C ${CHANNEL_NAME} --name ${CHAINCODE_NAME} \
      --peerAddresses localhost:7051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG1 --peerAddresses localhost:9051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG2 -c '{"Args":["CreateActor", "1", "rawMaterial","James JohnsonX9", "{}"]}'
   echo "===================== Successfully Added New Actor===================== "
}

chaincodeQueryActorById() {
   echo "===================== Started Query Actor By Id Chanicode Function===================== "
   setEnvVarsForPeer0Org1
   peer chaincode invoke -o $ORDERER_ADDRESS --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C ${CHANNEL_NAME} --name ${CHAINCODE_NAME} \
      --peerAddresses localhost:7051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG1 --peerAddresses localhost:9051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG2 -c '{"Args":["QueryActor", "1"]}'
   echo "===================== Successfully Invoked Query Actor By Id Chanicode Function===================== "
}
...
chaincodeTransferPropertyOwnership() {
   ...
   setEnvVarsForPeer0Org1
   peer chaincode invoke -o $ORDERER_ADDRESS --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C ${CHANNEL_NAME} --name ${CHAINCODE_NAME} \
      --peerAddresses localhost:7051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG1 --peerAddresses localhost:9051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG2 -c '{"Args":["TransferProperty", "1","Mark"]}'
